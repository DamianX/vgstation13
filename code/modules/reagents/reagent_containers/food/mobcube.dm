/obj/item/weapon/reagent_containers/food/snacks/monkeycube
	...
	var/mob/living/carbon/contained_mob = /mob/living/carbon/monkey // Either a path or an instance of mob/living/carbon
...
/obj/item/weapon/reagent_containers/food/snacks/monkeycube/Destroy()
	if(contained_mob && !ispath(contained_mob))
		qdel(contained_mob)
		contained_mob = null
	..()
...
/obj/item/weapon/reagent_containers/food/snacks/monkeycube/after_consume(var/mob/M)
	...
	if (istype(M, /mob/living/carbon/human))
		...
		surprise.icon = initial(contained_mob.icon)
		surprise.icon_state = initial(contained_mob.icon_state)
		...
		if(ispath(contained_mob))
			target_mob = new contained_mob(get_turf(M))
		else
			target_mob = contained_mob
			target_mob.forceMove(get_turf(M))
		target_mob.name = "malformed [initial(name)]"
		target_mob.transform *= 0.6
		target_mob.add_blood(M)
		M.gib()
	..()
