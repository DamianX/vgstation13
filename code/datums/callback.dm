...
/world/proc/ImmediateInvokeAsync(var/thingtocall, var/proctocall, ...)
	set waitfor = FALSE
...
/datum/callback_select/New(var/count, var/savereturns)
	total = count
	if (savereturns)
		finished = new(count)


/datum/callback_select/proc/invoke_callback(var/index, var/datum/callback/callback, var/list/callback_args, var/savereturn = TRUE)
	set waitfor = FALSE
	if (!istype(callback))
...
/proc/callback_select(var/list/callbacks, var/list/callback_args, var/savereturns = TRUE, var/resolution = 1)
	if (!callbacks)
		return
