...
/datum/component/proc/RegisterSignal(var/datum/target, var/sig_type_or_types, var/proc_or_callback, var/override = FALSE)
	...
	if(!istype(proc_or_callback, /datum/callback)) // If it wasn't a callback before
		proc_or_callback = CALLBACK(src, proc_or_callback) // it is now
...
/datum/proc/SendSignal(var/signal_kind, var/list/arguments)
	var/target = comp_lookup[signal_kind]
	if(!length(target))
		var/datum/component/C = target
		if(!C.enabled)
			return NONE
		var/datum/callback/CB = C.signal_procs[src][signal_kind]
...
/datum/proc/GetComponents(var/c_type)
	var/list/dc = datum_components
	if(!dc)
		return null
	. = dc[c_type]
