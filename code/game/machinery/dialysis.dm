...
/obj/machinery/dialysis/RefreshParts()
	...
	reagent_removal_per_tick = initial(reagent_removal_per_tick) * lasercount
...
/obj/machinery/dialysis/MouseDropFrom(over_object, src_location, over_location)
	if(usr.incapacitated() || ishigherbeing(usr) || !usr.Adjacent(src))
		return ..()
	...
	if(ismob(over_object) && Adjacent(over_object))
		if(ishuman(over_object))
			...
			if(H.species && (H.species.chem_flags & NO_INJECT))
				H.visible_message("<span class='warning'>[usr] struggles to place the IV into [H] but fails.</span>")
				return
...
/obj/machinery/dialysis/process()
	...
	for(var/mob/living/M in connections)
		...
		if(!M.reagents.remove_any(reagent_removal_per_tick))
			if(prob(35))
