...
/obj/machinery/computer/shuttle_control/Topic(href, href_list)
	...
	if(href_list["link_to_shuttle"])
		...
		var/choice = input(usr,"Select a shuttle to link this computer to", "Shuttle control console") in L
		if(!Adjacent(usr) && !isAdminGhost(usr) && !isAI(usr))
			return
	...
	if(href_list["admin_link_to_shuttle"])
		...
		var/choice = input(usr,"Select a shuttle to link this computer to", "Admin abuse") in L
		if(L[choice] && istype(L[choice],/datum/shuttle))
			shuttle = L[choice]
